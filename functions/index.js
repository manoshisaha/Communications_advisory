...
// Injects ALL CMS data server-side: hero, settings, logos, gallery
...
export async function onRequest(context) {
  ...
  // Handles BOTH "  - " (Decap indent) and "- " list formats
  ...
  if (heroYml) {
    const h=extractVal(heroYml,'headline'), s=extractVal(heroYml,'subheading'),
          p=extractVal(heroYml,'btn_primary'), b=extractVal(heroYml,'btn_secondary');
  ...
  if (settYml) {
    const email=extractVal(settYml,'email'), li=extractVal(settYml,'linkedin'),
          tw=extractVal(settYml,'twitter'), fb=extractVal(settYml,'facebook');
    ...
    if (items.length) {
      ...
      const renderItem = (item,i) => {
        const img = item.image
          ...
          : '';
        return `<div class="gallery-item ${g[i%g.length]}">${img}<div class="gallery-caption"><div class="gallery-caption-title">${esc(item.title||'')}</div><div class="gallery-caption-sub">${esc(item.subtitle||'')}</div></div></div>`;
      };
